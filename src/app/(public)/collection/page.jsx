            ...
            <section className="relative w-full py-20 bg-slate-50">
                <div className="px-4 md:px-10 flex flex-1 justify-center">
                    <div className="layout-content-container flex flex-col max-w-[1200px] flex-1 text-center items-center">
                        <span className="px-4 py-1.5 rounded-full bg-white border border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 shadow-sm">
                            Thư viện hình ảnh
                        </span>
                        ...
                        <p className="text-slate-600 text-lg md:text-xl font-light max-w-2xl leading-relaxed">
                            Lưu giữ những giây phút tuyệt vời nhất tại Airwave Beach Club. Nơi vẻ đẹp, âm nhạc và cảm xúc hòa quyện.
                        </p>
                    </div>
                </div>
            </section>
                    ...
                    <div className="layout-content-container flex flex-col max-w-[1200px] flex-1">
                        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
                            {galleryImages.map((img, index) => (
                                <div key={index} className="break-inside-avoid relative group rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-500 cursor-pointer">
                                    <img
                                        src={img.src}
                                        alt={img.alt}
                                    ...
                                    {/* Optional: Caption or tag */}
                                    <div className="absolute bottom-4 left-4 transform translate-y-12 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                                        <span className="px-3 py-1 bg-white/90 backdrop-blur-md rounded-full text-xs font-bold text-slate-900">
                                            {img.alt}
                                        </span>
                                    </div>
                                </div>
            ...
            <section className="py-20 px-4 bg-slate-900 text-white">
                <div className="max-w-[1200px] mx-auto text-center">
                    <h2 className="text-3xl md:text-5xl font-black mb-6">
                        Bạn đã sẵn sàng để trở thành một phần <br />của bức tranh này?
                    </h2>
                    <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto">
                        Hãy đến và tạo nên những kỷ niệm đáng nhớ của riêng bạn tại Airwave Beach Club.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link href="/contact" className="inline-flex items-center justify-center h-14 px-8 rounded-full bg-primary hover:bg-primary-hover text-slate-900 font-bold transition-all shadow-lg shadow-green-500/20">
                            Liên hệ đặt bàn
                        </Link>
                        <Link href="/menu" className="inline-flex items-center justify-center h-14 px-8 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white text-white font-bold transition-all backdrop-blur-sm">
                            Xem thực đơn
                        </Link>
                    </div>
                </div>
